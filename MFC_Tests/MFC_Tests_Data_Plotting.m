...
% Part 1 - Date, Board, and Chip 
%   Format: YYYY-MM-DD_Bx_Cx
...
switch lower(asksave)
    case {"y",''}
        ...
        chippath = "/Board"+num2str(target_board)+"_Chip"+num2str(target_chip)+"/";
        ...
        for f = 1:length(all_figs)
            saveas(all_figs(f),fullfile(savepath, all_figs(f).FileName),'png');
        end
